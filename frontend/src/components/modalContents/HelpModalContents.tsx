...
import { getLocalStorageOrDefault } from '../../utils/localStorageHelpers';
...
const HelpModalContents = (props: Props) => {
  ...
  const handleGetPreviousWords = () => {
    const words = getLocalStorageOrDefault('vocablist', '(no previous input found)');
    console.log(words);
    const strippedWords = words.replaceAll('"', '');
    setTextInput(strippedWords);
    props.handleClose();
  }
