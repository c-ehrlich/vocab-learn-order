...
const frequencyListWeightsSchema = object(
  {
    animeJDrama: number({
      required_error:
    ...
    vn: number({
      required_error: 'Frequency List weighting for Visual Novels is required',
      invalid_type_error: 'Frequency List weightings must be strings',
    }),
    wikipedia: number({
...
export type TWord = TypeOf<typeof createWordSchema>;
export type TFrequencyListWeights = TypeOf<typeof frequencyListWeightsSchema>;
export type TSearchWordsInput = TypeOf<typeof searchWordsSchema>['body'];
